...
## 紀律 · KOL 系統方向 C 整合(2026-04-30 教練蓋章)

### 核心紀律

**Phase 0:KOL 系統維持現狀 · 不砍不重啟 · Phase 1 啟動前(2026/06 中)升級為 Web3-native(方向 B)。**

詳細戰略 / 兩套並存系統(KOL 大使 vs Genesis 信徒)/ 升級 roadmap → **見 STATUS.md「KOL 系統 · 方向 C 整合」章節**(避免本檔重複)。

### 紅帽 / 未來自己必守(三條紅線)

```
❌ 不砍 KOL 系統
   有真實 bronze holder 付 200 USDT · 砍 = 違約
   Phase 1+ 重構 Web3-native 也要靠這個 schema 地基

❌ 不重啟 Ambassador 招募
   AMBASSADOR_SECTION_VISIBLE = False(2026-04-18 Polar/Stripe 審核前置)
   Phase 0 維持隱藏 · 不為短期流量恢復
   Phase 1 升級 Web3-native 後再公開

❌ 不混用 candle/skylamp 跟 bronze/silver/gold/diamond
   兩套並存的 enum · 共用 KolAccount.genesis_tier 欄位
   stats endpoint 要兩套一起算(2026-04-30 已修)
   寫新 SKU 前先 grep 兩個 list · 確認語義
```

### 對應 commit / file

- [`app/api/kol.py:670`](app/api/kol.py:670) · `/kol/genesis-stats` 修 enum bug(by_tier 6 keys)
- [`app/api/genesis.py:179`](app/api/genesis.py:179) · `/genesis/mint-count` 新建(Live Counter 用 · 對齊 Final v1.0 130 席)
- [`docs/Genesis-100-Final-Spec-v1.md`](docs/Genesis-100-Final-Spec-v1.md) · 定案 v1.0 落地(2026-04-30 教練蓋章)
